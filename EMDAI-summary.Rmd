...
rocs <- allpredmeasures %>%
  ...
  summarize(max_test_auc = roc(value,predval)$auc,
...
1) A set of key data quality indicators which represent the main sources of missingness in our data, which must be minimized if valid predictive models are to be developed, finding that missingness is associated with patient acuity, with both high and low priority calls having higher rates of missing data.

2) A selection of measures chosen from among the 19 measures presented in the white paper, representing important aspects of patient care needs with regards to Ambulance interventions, Vital signs, and Hospital outcomes, finding that dispatchers achieve the highest rates of differentiation with regards to measures of pre-hospital and in-hospital acute care.

3) A summary of the performance of the existing rule-based decision support system (MBS), actual dispatched priorities, and our preliminary models with regards to predicting the occurrence of the measures presented above. We find that our models are likely to provide the most value to dispatchers in predicting measures of sub-acute care.
...
It remains before us to further improve these models and improve the quality of our data, and develop adaptations to take consideration of the non-random pattern of missingness. We are also actively working to develop methods to aggregate and present model predictions to nurses in a useful and interpretable manner. This document will be updated as we continue our work in the project. Once we feel that a sufficiently robust approach has been developed, we will validate our methods in a prospectively gathered dataset which we have not observed, and submit the results to a peer-reviewed journal for publication.