...
abstract: Triage tools based on Machine Learning offer a promising approach to enhancing
  the decisional capacity of staff working at Emergency Medical Dispatch centers.
  There are however many challenges to implementing such systems including the definition
  of relavant outcomes and patient cohorts, handling issues relating to data quality,
  and choosing modelling techniques. In a 2 year project funded by the Swedish Agency
  for Innovation, the Uppsala Ambulance Service aims to develop a machine-learning
  based tool to augment the current rule-based decision support system. In this analysis,
  we outline our approach to addressing these challenges, provide a thorough descriptive
  analysis of the data available for this purpose, and provide a brief report regarding
  the overall performance of a set of preliminary models based on retrospective data.
  Overall, our current non-optimized models have a fair predictive value across a
  broad range of outcomes including ambulance interventions, patient vital signs,
...
# Function to converd Dxy to AUC
...
The AUC value provides a useful overall summary, but is also problematic in a number of ways. AUC values will tend to be higher in cases where the outcome is rare, and predictors with fewer levels will have artificially low AUC values. Being a summary, the AUC includes combinations of sensitivity and specificity which may be unimportant clinically. For instance, having anything less than an extremely high degree of sensitivity for cardiac arrests would be unacceptable. While better measures of overall predictive value do exist (the area under a precision-recall curve for instance), for better or worse, AUC is the measure which has become the "industry standard", and which we will report in this analysis.

[Levin et al. (2017)](https://doi.org/10.1016/j.annemergmed.2017.08.005) used Random Forest models to achieve predictive values of 0.73 - 0.92 AUC for a number of outcomes using data gathered at the time of ED triage. [Hong et al. (2018)](https://doi.org/10.1371/journal.pone.0201016) meanwhile achieved an AUC of 0.92 in the prediction of hospital admission by integrating data from the patient's historical medical records using a number of methods. [Horng et al. (2017)](https://doi.org/10.1371/journal.pone.0174708) used support vector machines to analyze free-text and clinical data, achieving an AUC of 0.86 in detecting diagnosed infections. [Tadahiro et al. (2018)](https://doi.org/10.1016/j.ajem.2018.06.062) were able to achieve AUC values of 0.80 and 0.83 respectively in predicting critical care and admission among Asthma and COPD patients in the ED. 
...
Clearer trends are seen with regards to NEWS scores - While Priority 1A calls do have higher NEWS scores, there is little differentiation among the remainder of the call priorities. Of particular interest is the performance of dispatchers with regards to referred patients. Here the effects found above are quite interesting - Among calls where the MBS priority was followed, NEWS scores among referred patients are similar to priority 2 calls and do not differ between patients referred to alternate care or transport. Among calls where the priority was downgraded by the dispatcher however, calls referred to alternate transport have NEWS scores on par with priority 1 calls, while calls referred to alternate care have far lower NEWS scores - Indeed, there were zero manually downgraded calls where the NEWS score exceeded 7! This could be a sign that features to allow the MBS to differentiate between conditions requiring transport vs. conditions where a referral to primary care is sufficient would be useful.
...
Since these patient characteristics were not included as predictors when imputing missing NEWS scores, we can use the somewhat idiosyncratic distributions to verify that our multiply imputed NEWS values are distributed appropriately:
...
While this model has the benefit of being readily interpretable (coefficient estimates are the effect in log-odds of a single unit change in the predictor), it leaves much to be desired. As we saw previously, several of these variables are non-linear, and we need to handle them as such.  
...
We see that we've achieved a much improved model calibration as well. Note that cross-validation can give some wonky results in low-frequency regions as seen above. This model can also tell us something about the risks associated with these predictors:
...
Using gradient boosting, we can begin to explore the predictive value of our other datasets. We can plot similar training logs for models containing each of the sets of predictors noted in section 2. In addition to the call and patient characteristics (What we call out "Null" predictors) and call types, we also add the answers to the MBS questions, coded using -1 for a negative answer, 1 for a positive answer, and 0 for a missing answer.
...
We see that each additional predictor adds value to the model. We see that for the simpler models, gradient boosting quickly maximizes it's ability to predict the outcome, with additional iterations only increasing the training AUC. For others, additional training iterations could lead to small gains in cross-validated performance.
