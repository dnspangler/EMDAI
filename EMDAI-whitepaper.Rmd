...
In the plot of randomly generated data above, the black line represents the predictive value of the model - in this case, an AUROC of 0.74. Random guessing would result in an AUROC of 0.5 and a curve laying flat along the red line, while a model making perfect predictions would have an AUROC of 1. From this plot, any given sensitivity can be selected and the corresponding specificity read off - Selecting a sensitivity of 0.9 here results in a specificity of roughly 0.35 (Equaivalent to a False Positive Rate (FPR) of 0.65, indiated by the blue dot in the above plot).
...
# Generate stopword lists

# swe_stop <- read.delim("https://gist.githubusercontent.com/hannseman/5608626/raw/7b11a75d393a68d0145bdcefb06fc06d2764daa8/swedish_stopwords.txt",
#                        header=F, encoding = "UTF-8", stringsAsFactors = F) 
# 
# # Define negation words to exclude from stopword list for n-grams
# swe_stop_ngram <- swe_stop %>%
#   filter(!V1 %in% c("inte","ej","icke","blir","bli","blev","blivit","mycket","nu","utan","var"))
# 
# # Define regex friendly list of stopwords
# swe_stop_reg <- paste0(" ",paste(swe_stop$V1,collapse = " | ")," | *\\b[[:alpha:]]{1}\\b *")
# swe_stop_ngram_reg <- paste0(" ",paste(swe_stop_ngram$V1,collapse = " | ")," | *\\b[[:alpha:]]{1}\\b *")
#
# # Use tidytext to Tokenize and convert to word matrix (Should make this use a function really)
#
# ft <- fulldata %>%
#   dplyr::select(caseid,freetext) %>%
#   mutate(cleanft = tolower(freetext)) %>%
#   mutate(cleanft = str_replace_all(cleanft, "[^[:alpha:]]"," ")) %>%
#   dplyr::select(caseid,cleanft)
# 
# cleanwords <- ft %>%
#   mutate(cleanft = str_replace_all(cleanft, swe_stop_reg, " "),
#          type = "word") %>%
#   unnest_tokens(term, cleanft, token = "words") %>%
...
# 
# cleanbigrams <- ft %>%
#   mutate(cleanft = str_replace_all(cleanft, swe_stop_ngram_reg, " "),
#          type = "bigram") %>%
#   unnest_tokens(term, cleanft, token = "ngrams",n = 2) %>%
#   group_by(caseid,term) %>%
#   mutate(docfreq = n()) %>%
#   group_by(term) %>%
#   mutate(totfreq = n()) %>%
...
#   filter(totfreq > 99)
# 
# cleantokens <- rbind(cleanwords, cleanbigrams) %>%
#   filter(!is.na(term)) %>%
#   bind_tf_idf(caseid,term,docfreq)
# 
# wordmat <- cleantokens %>%
#   distinct() %>%
#   dcast(caseid ~ term, value.var = "tf_idf") %>%
#   mutate_all(funs(ifelse(is.na(.),0,.)))
# 
# names(wordmat)[-1] <- paste0("t_",names(wordmat)[-1])
#
# save(wordmat, file = "wordmat.rda")

load("wordmat.rda")

alldata <- left_join(alldata, wordmat, by = "caseid") %>%
  mutate_at(vars(starts_with("t_")),funs(ifelse(is.na(.),0,.)))
...
# Generate aggregate word ratios

# cats <- select(alldata,starts_with("cat_"))
# 
# cats <- lapply(cats, function(x){which(x == 1)})
# 
# catids <- lapply(cats, function(x){alldata$caseid[x]})
# 
# topcat <- catids[which(sapply(catids, length) > 1000)]
# 
# catids$cat_Full <- alldata$caseid
# 
# outobs <- c(rep(1,sum(grepl("amb_",names(txtout)))),
#             rep(2,sum(grepl("v_",names(txtout)))),
#             rep(3,sum(grepl("hosp_",names(txtout)))))
# 
# Looks like I stored some outcome data in a seperate df here... Oops, might fix this to use alldata later...
# names(outobs) <- names(txtout)[-1]
# 
# desctokens <- cleantokens %>%
#   left_join(.,txtout,by="caseid")
# 
# wordratios <- data.frame(term = character(0),
#                          ct = character(0),
#                          out = character(0),
#                          stringsAsFactors = F)
# 
# for(i in 1:length(outobs)){
#   
#   for(j in 1:length(topcat)){
#     
#     f <- desctokens %>% 
#       filter(caseid %in% topcat[[j]] &
#                caseid %in% obs[[outobs[i]]]) %>%
#       group_by_(names(outobs[i])) %>% 
#       count(term, sort = TRUE) %>% 
#       left_join(desctokens %>% 
#                   group_by_(names(outobs[i])) %>% 
#                   summarise(total = n())) %>%
#       mutate(freq = n/total) %>% 
#       dplyr::select_(names(outobs[i]), "term", "freq") %>% 
#       spread_(names(outobs[i]), "freq", fill = 0) %>%
#       mutate(ct = names(topcat[j]),
#              out = names(outobs[i]))
#     
#     if(ncol(f) == 5){
#       
#       wordratios <- bind_rows(wordratios,
#                               f)
#       
#     }
#   }
# }
#
# save(wordratios, "180903_wordratios.rda")
...
# svd <- svd(as.matrix(dplyr::select(alldata,starts_with("t_"))))
# 
# save(svd, file = "181124_svd.rda")
...
lab <- moddata[ambptobs,which(dimnames(moddata)[[2]] == "amb_any")]
...
learning_curve <- function(fulldat, 
  ...
  for(i in 1:length(breaks)){
    samp <- sample(trainobs,breaks[i])
    dat <- fulldat[samp,]
    lab <- fulllab[samp]
    mod <- do.call(xgboost,testparams)
    trainerr <- mod$evaluation_log[nrow(mod$evaluation_log),2]
    devpred <- ifelse(predict(mod, fulldat[devobs,]) >= 0.5,1,0)
    deverr <- sum(devpred != fulllab[devobs])/length(fulllab[devobs])
    results[i,] <- cbind(n = breaks[i],trainerr,deverr)
  }
  return(results)
}

lc <- learning_curve(fulldat = dat,
                     fulllab = lab,
...
lc %>%
  ...
  labs(x = "Sample size",
       y = "Error",
       color = "Dataset",
       title = "Learning curve for hospital admission model")

```

We see that the error in our models in out-of-samle data generally plateaus at an error rate of around 0.3 after ~10 000 observations, and that the error is due to bias in the model, rather than high variance. As such, adding additional data to our model is unlikely to improve performance, and that the data could likely support a more complex model.

We also need to check that our model is well calibrated - That is to say, that the admission risks predicted by the model lack systematic bias across the range of predicted values. We can do this using the cross validation approach we used earlier, and we'll use the the RMS package calibrate function to do the prior.

```{r}

xgbadmit_cv <- do.call(xgb.cv,testparams)
tdf <- data.frame(out = lab,pred = xgbadmit_cv$pred)
lrm <- lrm(out ~ pred,data = tdf, x=T,y=T)
plot(calibrate(lrm,method = "crossvalidation",B=10))
...
```{r}
refobs <- which(alldata$pout[hospptobs] == "Referral")

tdf_ref <- data.frame(out = lab[refobs],pred = xgbadmit_cv$pred[refobs])
lrm <- lrm(out ~ pred[],data = tdf_ref, x=T,y=T)
plot(calibrate(lrm,method = "crossvalidation",B=10))
```
...
Finally, we're interested in assessing whether these models have the ability to provide a clinical benefit to patients who are triaged based on model findings. To do this, we can perform a [decision curve analysis](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2577036/) to evauate the potential benefit of a predictive model across various risk thresholds.

```{r}

model_admit <- decision_curve(out ~ pred,
                       data = tdf, 
                       study.design = "cohort", 
                       policy = "opt-in",
                       bootstraps = 50)

plot_decision_curve(model_admit,
                    curve.names = "model",
                    standardize = F,
                    main = "Decision curve - Full population")

model_admit_ref <- decision_curve(out ~ pred,
                       data = tdf_ref, 
                       study.design = "cohort", 
                       policy = "opt-in",
                       bootstraps = 50)

plot_decision_curve(model_admit_ref,
                    curve.names = "model",
                    standardize = F,
                    main = "Decision curve - Referred patients")

```

We see that in the plots above, the net benefit of applying the model to determine whether a patient will be admitted to the hopsital beings to diverge in terms of net benefit from assuming that all patients will be admitted to the hospital at threshold value corresponding to 1:7 among the full population, and 1:16 among referred patients only. 

Thus the questions becomes a clinical and operational one: How much worse is it to refer a patient who should be admitted to primary care, than to send a patient who doesn't need in-patient care to the hospital? If the answer to this question is "Less than 16 times as bad among referred patients", then this model may be able to deliver a clinical benefit.
