...
Associated [source code](https://github.com/dnspangler/EMDAI/) is licensed under an [Apache License 2.0](http://www.apache.org/licenses/LICENSE-2.0)
...
In the plot of randomly generated data above, the black line represents the predictive value of the model - in this case 0.74. Random guessing would result in an AUC of 0.5 and a curve laying flat along the red line, while a model making perfect predictions would have an AUC of 1. From this plot, any given sensitivity can be selected and the corresponding specificity read off - Selecting a sensitivity of 0.9 here results in a specificity of 0.4.

The AUC value provides a useful overall summary, but is also problematic in a number of ways. AUC values will tend to be higher in cases where the outcome is rare, and predictors with fewer levels will have articifially low AUC values. Being a summary, the AUC includes combinations of sensitivity and specificity which may be unimportant clinically. For instance, having anything less than an extremely high degree of sensitivity for cardiac arrests would be unacceptable. While better measures of overall predictive value do exist (the area under a precision-recall curve for instance), for better or worse, AUC is the measure which has become the "industry standard", and which we will report in this analysis.

[Levin et al. (2017)](https://doi.org/10.1016/j.annemergmed.2017.08.005) used Random Forest models to achive predictive values of 0.73 - 0.92 AUC for a number of outcomes using data gathered at the time of ED triage. [Hong et al. (2018)](https://doi.org/10.1371/journal.pone.0201016) meanwhile achieved an AUC of 0.92 in the prediction of hospital admission by integrating data from the patient's historical medical records using a number of methods. [Horng et al. (2017)](https://doi.org/10.1371/journal.pone.0174708) used support vector machines to analyze free-text and clinical data, achieving an AUC of 0.86 in detecting diagnosed infections. [Tadahiro et al. (2018)](https://doi.org/10.1016/j.ajem.2018.06.062) were able to achieve AUC values of 0.80 and 0.83 respectively in predicting critical care and admission among Asthma and COPD patients in the ED. 
...
cats <- dplyr::select(alldata[alldata$ambj == 1 &
...
We see that the missingness of vital signs varies across call types! Note also the high rate of missingness of AVPU determinations due to many of our clinicians preferring to document patient alertness in terms of the Glasgow Coma Scale. We also see that some specific vitals are missing from certain call types, e.g., high rates of missing temperatures from trauma calls.
...
Note that the number of interventions presented here has been reduced to maintain legibility. The results among ambulance interventions are quite heterogeneous, with no clear overall trends. Some interesting effects can however be identified - It appears that with regards to calls where the MBS was not used for instance, priority 2 calls appear to have higher rates of transport.
...
Clearer trends are seen with regards to NEWS scores - While Priority 1A calls do have highler NEWS scores, there is little differentiation among the remainder of the call priorities. Of particular interest is the performance of dispatchers with regards to referred patients. Here the effects found above are quite interesting - Among calls where the MBS priority was followed, NEWS scores among referred patients are similar to priority 2 calls and do not differ between patients referred to alternate care or transport. Among calls where the priority was downgraded by the dispatcher however, calls referred to alternate transport have NEWS scores on par with priority 1 calls, while calls referred to alternate care have far lower NEWS scores - Indeed, there were zero manually downgraded calls where the NEWS score exceeded 7! This could be a sign that features to allow the MBS to differentiate between contiditons requiring transport vs. conditions where a referral to primary care is sufficient would be useful.
...
Among hospital outcomes we see that while differentiation with regards to hospital admission is low for all groups, the effect whereby low priority calls have higher rates of admission seems to be primarily localized to calls where the MBS was not used. 

These overall summary statistics can of course only provide an overview of the predictive value of the MBS decision rules and actual dispatched priorities in terms of various outcomes. On the whole, we see better differentiation between priorities for ambulance interventions than among hospital outcomes.
...
Since these patient characteristics were not included as predictors when imputing missing NEWS scores, we can use the somewhat ideosyncratic distributions to verify that our multiply imputed NEWS values are distributed appropriately:
...
alldata %>%
  ...
  dplyr::summarize(n = n(),
            ...
            `Ambulance intervention (%)` = formatpct(mean(ifelse(ambj == 1,
                                              amb_any,
                                              NA),
                                       na.rm = T)),
...
Most values seem to be what one might expect. Interesting that The "Intoxication" call type has such a high rate of ICU care!

### MBS data

In addition to the initial categorization, dispatch nurses are to complete a battery of questions, the answers to which can result in an upgraded dispatch priority. As noted previously, only the relatively well-documented _ABCDE_ questions have been found to be of use for the purposes of predictive modelling. As the structured triage system and the questions it contains are protected by copyright, we unfortunately cannot provide a detailed breakdown of the distribution of answers to these questions.
...
We'll begin by including each of these characteristics as a simple linear predictor in such a model:
...
ambmoddata <- alldata[ambptobs,] %>%
  dplyr::select(amb_any,null_age,null_female,null_pcontacts,null_lastcontact,hour,null_time_sin,null_time_cos, month, null_month_sin, null_month_cos, null_wkday, wkday, starts_with("cat_"), starts_with("q_"),caseid) %>%
...
By applying a bit of feature engineering we've improved the fit of the model and it's calibration. These factors still only account for 2.7% of the variation in the data however, and has quite poor predictive value with a C-Index of 0.58. 

We can examine how our data is likely to perform on new datasets using cross validation - This involves splitting our data into a number of sub-sets (10 in this case) to predict the datapoints not selected in each sample. Let's compare how our original and somewhat improved model fare in terms of calibration:

```{r lrmcal}
cal1 <- calibrate(nulllrm, 
                  method = "crossvalidation",
                  B = 10,
                  subtitles = F)
cal2 <- calibrate(nullnllrm, 
                  method = "crossvalidation", 
                  B = 10,
                  subtitles = F)

par(mfrow=c(1,2))
plot(cal1, main="Original")
plot(cal2, main="Engineered")
```

We see that we've achieved a much improved model calibration as well. Note that crossvalidation can give some wonky results in low-frequency regions as seen above. This model can also tell us something about the risks associated with these predictors:

```{r nlpreds}


pc <- ggplot(Predict(nullnllrm,null_pcontacts))  +
  labs(title= "Estimated risk of ambulance intervention",
       x = "Number of prior contacts",
       caption = "")

age <- ggplot(Predict(nullnllrm,null_age))  +
  labs(title= "Estimated risk of ambulance intervention",
       x = "Patient age",
       caption = "")
...
We see a similar effect with regards to ambulance interventions as we did with NEWS values, though not quite as dramatic. Interestingly, intervention rates by age appears to plateau after age 80 or so.
...
We find that we achieve an improved C-index of 0.68 and corresponding increases in measures of model fit. At this point, we should perhaps begin worrying about overfitting - Are we being too confident in our predictions given the large number of predictors? In terms of overall model predictions, we can investigate this using cross validation to estimate the out-of-sample performance, finding a C-index in the test folds of `r round(dxy2auc(validate(catnllrm, method = "crossvalidation", B = 10)[1,3]),3)`, suggesting that overfitting is not yet an issue.

Using this model, we can also investigate the effect of each call type on intervention probability with patient characteristecs controlled for. Given the relatively large number of call types, we should be careful about interpreting individual coefficients however - To limit the number of estimates we present, we can apply a Bonferroni correction, then make it 100 times more extreme (p < 0.000006), and take a look at the odds ratios associated with each call type, using cases where MBS was not used as a comparison group:
...
We find that childbirth, chest pain and active seizure calls have the highest intervention rates, with odds ratios of around 5. At the low end, we see that calls related to urinary catheter blockages and urine retention, as well as calls relating to psychiatric problems have odds ratios of around 0.3.
...
To account for interaction effects, colinearity, and other issues which arise as we add increasingly complex forms of data to our set of predictors, we experimented with a number of techniques including random forest models, neural networks, and gradient boosting. While we will provide a comparison of the performance of these techniques in a future publication, it is not the goal of this analysis to determine an optimal modelling methods for use in our data. Rather, we are interested in exploring the general and relative value of our predictors in terms of estimating the likelihood of our outcomes of interest. In this way, we can use predictive models as an exploratory tool to understand the relationships present in our data.
...
Let's begin by re-fitting our last model using xgboost. We generate a plot of how the predictive value of the model changes as additional iterations improve model fit, indicated by the mean AUC found in the test and training folds of the data, and their standard errors:
...
Using gradient boosting, we can begin to explore the predictive value of our other datasets. We can plot similar training logs for models containing each of the sets of predictors noted in section 2. In addition to the call and patient characteristics (What we call out "Null" predictors) and call types, we also add the answers to the MBS questions, coded using -1 for a negative, anwer, 1 for a positive answer, and 0 for a missing answer.

In analysinzg text data, we've found that documentation practices vary from nurse to nurse at the EMD center, and taking consideration for which particular nurse is documenting the call appears to improve the predictive value of the free text data. We include this effect by generating dummy variables for each operator with more than 50 calls taken, and include it alongside the bag-of-words model described earlier. Finally, we include all of these predictors in a final model:
...
We see that each additional predictor adds value to the model. We see that for the simpler models, gradient boosting quickly maximizes it's ability to predict the outcome, with additonal iterations only increasing the training AUC. For others, additional training interations could lead to small gains in cross-validated performance.

### Model summaries

With an understanding of how these models are generated, let's have a look at how these models perform for each of our outcomes of interest. To summarize our results so far, we can extract the cross-validated AUC for each of the nested sets of predictors for each outcome. As noted previously, There are some issues with AUC as a measure of predictive value, but we will use it nonetheless given it's prevalence in the literature. We can begin with ambulance interventions (error bars again indicate one standard error from the mean):
...
Each indicator improves steadily with the addition of additional predictors, with the exception of 3 day mortality which improves considerably upon the addition of text data, but does not suggest improvement by the addition of MBS questions. Recall that AUC is sensitive to unbalance in datasets (i.e., having large numbers of negative cases improves AUC), and as such, comparisons between outcomes here are unreliable.
...
Among NEWS and hospital outcome measures, these models outperform current prioritizations consistently. Overall, we find that we have quite high AUC values for some specific types of interventions, in general our models have AUC value of 0.7 to 0.8.

In addition to their use in EMD, similar models could be applied to predict hospital interventions based on data gathered by the ambulance. We can get a sense of how including these data might effect the predictive value of these models by investigating the subset of our patients who are transported by ambulance to an ED. By including only patients for whom we have enough vitals recorded to have a NEWS score imputed, we can also investigate the performance of our measures in comparison with this benchmark:
...
The overall predictive value of our models based on dispatch data is lower than that found in models based on data gathered at the ED. This is unsurprising, given that information based on visually observing or pysically interacting with the patient are typically unavailable at the time of ambulance dispatch. While [Hong et al. (2018)](https://doi.org/10.1371/journal.pone.0201016) achieved an impressive AUC of 0.92 for hospital admission, while we achieved an AUC of .75 using only data from the EMD center, and 0.77 upon including data from the ambulance. [Levin et al. (2017)](https://doi.org/10.1016/j.annemergmed.2017.08.005) on the other hand achieved an AUC of 0.82-0.84 for hospital admission, much closer to the results found here. Given this range of predictive values, it seems uncertain as to what an "acceptable" level of differentiation in this context is. While we had hoped for AUCs on the order of 0.8, lower levels of predictive value could be compensated for by sacrificing specificity (AKA over-triaging) in order to ensure the safety of decisions made based on model predictions.
